...
def substitute(expanded_half_block, s_boxes, sbox_index):
    ...
    for sindex in range(len(segments)):
        if len(segments[sindex]) != 6:
            raise ValueError("Segment length must be 6 bits")
        
        # Ensure segments[sindex] is a string before extracting substrings
        segment_str = ''.join(map(str, segments[sindex]))  # Convert list of integers to a string
        
        row = int(segment_str[0:2], 2)
        column = int(segment_str[2:6], 2)
...
def single_round_DES(right_half_block, round_key, expansion_permutation, s_boxes):
    expanded_block = permute(right_half_block, expansion_permutation)
    mixed_block = 0
    for i in range(len(expanded_block)):
        mixed_block |= expanded_block[i] ^ round_key[i] << (31 - i)
    substituted_block = substitute(mixed_block, s_boxes, 0)  # Pass the S-box index (0 in this example)
...
def des_encrypt(message, key, key_permutation_1, key_permutation_2, shifts_for_round_key_gen,
    ...
    for block in message:
        bv = block
        while bv != 0:
            LE = bv & 0xFFFFFFFFFFFFFFFF  # Mask for the left half (64 bits)
            RE = bv >> 64  # Shift to get the right half (64 bits)
            for round_key in round_keys:
                newRE = LE ^ single_round_DES(RE, round_key, expansion_permutation, s_boxes)
                LE = RE
                RE = newRE
            bv = (RE << 64) | LE  # Combine left and right halves back into a single block
        ciphertext.append(bv)
    return ciphertext
...
shifts_for_round_key_gen = [1, 1, 2, 2, 2, 2, 2, 2,
                            1, 2, 2, 2, 2, 2, 2, 1]

# Example usage
message = [string_to_bits("Hello, World!")]
key = string_to_bits("This is a key!")
encrypted_message = des_encrypt(message, key, key_permutation_1, key_permutation_2, shifts_for_round_key_gen,
                                expansion_permutation, s_boxes)
